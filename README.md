# 🛡️ Cybersecurity Labs Portfolio

> A curated collection of hands-on labs, CTF walkthroughs, and security tools built during my journey toward becoming an **Application Security (AppSec) Engineer**.

[![TryHackMe](https://img.shields.io/badge/TryHackMe-Swoyon-212C42?style=flat&logo=tryhackme)](https://tryhackme.com/p/swoyon)
[![PortSwigger](https://img.shields.io/badge/PortSwigger-Learner-FF6633?style=flat)](https://portswigger.net/web-security)
[![Student](https://img.shields.io/badge/🎓-Student-00d4aa?style=flat)]()

---

## 📊 Progress Overview

| Platform | Labs Completed | In Progress |
|----------|---------------|-------------|
| **TryHackMe** | 4 | 2 |
| **PortSwigger** | 3 | 2 |
| **Custom Tools** | 1 | 1 |

---

## 📁 Lab Index

### 🎯 TryHackMe

| # | Room Name | Category | Difficulty | Status | Write-up |
|---|-----------|----------|------------|--------|----------|
| 01 | [Brooklyn Nine Nine](./TryHackMe/brooklyn-nine-nine) | CTF / Commands | Easy | ✅ Solved | [View](./TryHackMe/brooklyn-nine-nine/README.md) |
| 02 | [Bolt Room](./TryHackMe/bolt-room) | Web / CTF | Easy | ✅ Solved | [View](./TryHackMe/bolt-room/README.md) |
| 03 | [Hacker Bounty](./TryHackMe/hacker-bounty) | CTF / Web | Easy | ✅ Solved | [View](./TryHackMe/hacker-bounty/README.md) |
| 04 | [Pickle Rick](./TryHackMe/pickle-rick) | CTF / Linux | Easy | ✅ Solved | [View](./TryHackMe/pickle-rick/README.md) |
| 05 | [Network Services](./TryHackMe/network-services) | Networking / SMB / Telnet / FTP / SSH | Medium | ✅ Solved | [View](./TryHackMe/network-services/README.md) |

### 🕷️ PortSwigger Web Security Academy

| # | Lab Name | Vulnerability | Difficulty | Status | Write-up |
|---|----------|---------------|------------|--------|----------|
| 01 | [SQL Injection — Hidden Data Retrieval](./PortSwigger/sqli-hidden-data) | SQL Injection (WHERE clause) | Apprentice | ✅ Solved | [View](./PortSwigger/sqli-hidden-data/README.md) |
| 02 | [SQL Injection — Login Bypass](./PortSwigger/sqli-login-bypass) | SQL Injection | Apprentice | ✅ Solved | [View](./PortSwigger/sqli-login-bypass/README.md) |
| 03 | [SQL Injection — UNION Attack](./PortSwigger/sqli-union-columns) | SQL Injection | Practitioner | ✅ Solved | [View](./PortSwigger/sqli-union-columns/README.md) |

### 🛠️ Tools & Scripts

| # | Tool Name | Language | Purpose | Status | View |
|---|-----------|----------|---------|--------|------|
| 01 | [Domain to IP](./Custom_Tools/domain_to_ip) | Python | DNS Resolution Utility | ✅ Complete | [View](./Custom_Tools/domain_to_ip/README.md) |
---

## 🎓 About Me

**Saber Hasan Swoyon** | Software Engineering Undergraduate

- 🛡️ **Aspiring Application Security (AppSec) Engineer**
- 🐍 **Tech Stack:** Python | Linux | Networking | Web Security
- 📚 **Currently Learning:** Web Application Security (PortSwigger Web Security Academy)
- 🌏 **Languages:** 🌐 English | 中文 (HSK4) | বাংলা 
- 🎯 **Goal:** Master's in Cybersecurity → AppSec Career 

## 🛠️ Tech Stack

```
OS:        Kali Linux (VirtualBox)
Browser:   Firefox + FoxyProxy
Proxy:     Burp Suite Community Edition
Editor:    VS Code / Vim
Languages: Python, Bash, SQL
```

---

## 🤝 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/swoyon)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:swoyon@email.com)

---

> **Disclaimer:** All labs documented here are from authorized training platforms (TryHackMe, PortSwigger). These are shared for educational purposes only.

---

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=16&duration=3000&pause=1000&color=00FF41&center=true&vCenter=true&width=500&lines=Learning+by+doing.;One+lab+at+a+time.;Building+the+future+of+security." alt="Typing SVG" />
</p>
